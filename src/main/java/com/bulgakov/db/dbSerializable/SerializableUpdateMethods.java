 ...
 * @since 04.03.2017
 */
public class SerializableUpdateMethods {
    ...
    public static void updateCustomer(Object obj) {
        ...
        List<CustomerLight> listOfCustomers = (List<CustomerLight>) SerializableUtils.getRecords("Customers");
        ...
        if (canUpdate) {
            ...
            SerializableUtils.writeInFile(listOfCustomers, "Customers");
        }
    }

    /**
     * Method update User object. It checks for coincidence mail from other users and customers.
     * If new User has new Boss, method to do a check for the existence of a new boss in the db.
     * Can't change ID of object.
    ...
    public static void updateUser(Object obj) throws UnknownObjectType {
        ...
        if (!oldUser.getBoss().getPersonId().equals(newUser.getBoss().getPersonId())) {
            if (!SerializableUtils.checkForExistingId(newUser.getBoss().getPersonId())) {
                canUpdate = false;
            }else{
                if (!SerializableUtils.checkObjectType(newUser.getBoss().getPersonId(), "User")) {
                    canUpdate = false;
                }
            }
        }
    ...
    /**
     * Method update Project object. If new Project has new Customer, method to do a check for the existence of a new customer in the db.
     * Can't change ID of object.
    ...
    public static void updateProject(Object obj) throws UnknownObjectType {
        ...
        if (!oldProject.getCustomer().getPersonId().equals(newProject.getCustomer().getPersonId())) {
            if (!SerializableUtils.checkForExistingId(newProject.getCustomer().getPersonId())){
                canUpdate = false;
            }else{
                if (!SerializableUtils.checkObjectType(newProject.getCustomer().getPersonId(), "Customer")) {
                    canUpdate = false;
                }
            }
        }
    ...
    /**
     * Method update Sprint object. If new Sprint has new Project, method to do a check for the existence of a new Project in the db.
     * Can't change ID of object.
    ...
    public static void updateSprint(Object obj) throws UnknownObjectType {
        ...
        if (!oldSprint.getProject().getProjectId().equals(newSprint.getProject().getProjectId())) {
            if (!SerializableUtils.checkForExistingId(newSprint.getProject().getProjectId())) {
                canUpdate = false;
            }else{
                if (!SerializableUtils.checkObjectType(newSprint.getProject().getProjectId(), "Project")){
                    canUpdate = false;
                }
            }
        }
    ...
    public static void updateTask(Object obj) throws UnknownObjectType {
        ...
        // if new task has new parent task, method to do a check for the existence of a new parent task
        if (!oldTask.getParentTask().getTaskId().equals(newTask.getParentTask().getTaskId())) {
            parentTaskWasChange = true;
            if (!SerializableUtils.checkForExistingId(newTask.getParentTask().getTaskId())) {
                canUpdate = false;
            } else {
                if (!SerializableUtils.checkObjectType(newTask.getParentTask().getTaskId(), "Task")) {
                    canUpdate = false;
                }
            }
        }
        // if new task has new sprint, method to do a check for the existence of a new sprint
        if (oldTask.getSprint().getSprintId().equals(newTask.getSprint().getSprintId())) {
            if (!SerializableUtils.checkForExistingId(newTask.getSprint().getSprintId())) {
                canUpdate = false;
            } else {
                if (!SerializableUtils.checkObjectType(newTask.getSprint().getSprintId(), "Sprint")) {
                    canUpdate = false;
                }
            }
        }
    ...
    public static void updateDoTask(Object obj) throws UnknownObjectType {
        ...
        if ((!newDoTask.getDateBegin().equals(oldDoTask.getDateBegin())) || (!newDoTask.getDateEnd().equals(oldDoTask.getDateEnd()))) {
            List<DoTasksLight> listOfDoTask = (List<DoTasksLight>) SerializableUtils.getRecords("DoTasks");
            ...
            SerializableUtils.writeInFile(listOfDoTask, "DoTasks");
        }
    }
}
